...
function install (gyp, argv, callback) {
  ...
  function go () {
    ...
    mkdir(devDir, function (err, created) {
      ...
      var badDownload = false
        ...
        , extracter = tar.extract({ cwd: devDir, strip: 1, filter: isValid })
      ...
      function isValid (path, stat) {
        var name = path.substring(devDir.length + 1)
        var isValid = valid(name)
        if (name === '' && stat.type === 'Directory') {
  ...
  function eaccesFallback () {
    ...
    log.warn('EACCES', 'user "%s" does not have permission to access the dev dir "%s"', os.userInfo().username, gyp.devDir)
